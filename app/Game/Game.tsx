...
export default function Game() {
    ...
    useEffect(() => {
        ...
        const { S, O, V } = gameState.submittedSlots
        if (!(S && O && V)) return
        const allRight = S === stage.roles.S && O === stage.roles.O && V === stage.roles.V
        if (!allRight) return
        const t = setTimeout(() => {
            if (phase === 'tutorial') {
                setGameState(() => ({
            ...
            setGameState((prev) => {
                ...
                for (const piece of prev.pieces) {
                    ...
                    if (isPieceInRim(candidate)) {
                        const correctWord = st.roles[piece.type]
                        if (piece.word === correctWord) {
                            submittedSlots = { ...submittedSlots, [piece.type]: piece.word }
                            boxFlash = 'correct'
                            // piece consumed
                            continue
                        } else {
    ...
    const { S, O, V } = gameState.submittedSlots
    const stageCleared = S === stage.roles.S && O === stage.roles.O && V === stage.roles.V
        ...
        return (
            <div className={styles.gameContainer}>
                <div className={styles.gameScene} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', fontSize: '5rem', backgroundColor: '#2ecc71', color: 'white' }}>
                    <div>Correct!!</div>
                    <div style={{ fontSize: '2rem', marginTop: '2rem' }}>{stage.explanation}</div>
                    <div style={{ fontSize: '1.25rem', marginTop: '1.5rem' }}>{stage.englishExplanation}</div>
                </div>
            </div>
        )
    }
        ...
        <div className={styles.gameContainer}>
            <div className={styles.gameScene}>
                <div className={styles.BG}>
                    <div style={{ position: 'absolute', top: 10, left: 20, right: 20, fontSize: '2rem', color: '#333', display: 'flex', justifyContent: 'space-between', gap: '1rem' }}>
                    ...
                    <div style={{ position: 'absolute', top: 275, left: 500, right: 20, fontSize: '5rem', color: '#333'}}>
                        {ROLES.map((role) => (
                            <span key={role} style={{ color: 'black', marginLeft: '1rem' }}>
                                {gameState.submittedSlots[role] ?? '_'}
                            </span>
                        ))}
                    </div>
